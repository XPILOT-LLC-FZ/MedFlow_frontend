...
export default function ReceptionPrescriptionsPage() {
  ...
  const fetchPrescriptions = useCallback(async () => {
    ...
    try {
      ...
      toastError(locale === "ar" ? "فشل تحميل الوصفات الطبية" : "Failed to load prescriptions");
    } finally {
      setIsLoading(false);
    }
  }, [statusFilter, locale, toastError]);
  ...
  const handleSend = async (id: string) => {
    try {
      ...
      toastSuccess(locale === "ar" ? "تم إرسال الوصفة بنجاح" : "Prescription sent successfully");
      void fetchPrescriptions();
    } catch {
      toastError(locale === "ar" ? "فشل إرسال الوصفة" : "Failed to send prescription");
    }
  };
    ...
    <div className="p-4 lg:p-6 space-y-6 lg:space-y-8 w-full mx-auto bg-slate-50 min-h-screen pb-24 overflow-x-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-xl md:text-[22px] font-bold text-slate-900 tracking-tight">
            {locale === "ar" ? "الوصفات الطبية" : "Prescriptions"}
          </h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6 w-full lg:w-3/4 xl:w-2/3">
          ...
          <CardContent className="p-5 md:p-8 space-y-4 md:space-y-6">
            <div className="flex items-center justify-between">
              <span className="text-slate-900 font-bold text-xs md:text-sm">
                {locale === "ar" ? "معدل الكفاءة" : "Efficiency rate"}
              </span>
            ...
            <div className="space-y-3 md:space-y-4">
              <div className="flex items-baseline gap-2">
                <h3 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tighter">{efficiencyRate}%</h3>
                <span className="text-slate-400 text-[10px] md:text-sm font-bold truncate">
                  {locale === "ar" ? "وصفات تم إرسالها < 2س" : "prescriptions sent < 2h"}
                </span>
              </div>
        ...
        <Card className="border-none shadow-[0_8px_30px_rgb(0,0,0,0.02)] rounded-[24px] md:rounded-[32px] overflow-hidden bg-white">
          <CardContent className="p-5 md:p-8 space-y-4 md:space-y-6">
            <span className="text-slate-900 font-bold text-xs md:text-sm">
              {locale === "ar" ? "جاهزة للاستلام" : "Ready for pickup"}
            </span>
            <div className="space-y-3 md:space-y-4">
              <div className="flex items-baseline gap-2">
                <h3 className="text-2xl md:text-4xl font-black text-slate-900 tracking-tighter">{readyCount}</h3>
                <span className="text-slate-400 text-[10px] md:text-sm font-bold truncate">
                  {locale === "ar" ? "بانتظار استلام المريض" : "awaiting patient collection"}
                </span>
              </div>
              <div className="flex items-center">
                ...
                {readyCount > 3 && (
                  <span className="text-slate-400 text-[10px] md:text-xs font-bold ml-3">+{readyCount - 3} {locale === "ar" ? "أكثر" : "more"}</span>
                )}
              </div>
            </div>
          </CardContent>
        ...
        <CardContent className="p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8">
          ...
          <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 md:gap-6">
            <div className="relative w-full max-w-[400px]">
              ...
              <Input
                placeholder={locale === "ar" ? "البحث في المهام أو المرضى..." : "Search tasks or patients..."}
            ...
            <div className="flex items-center gap-2 md:gap-3 overflow-x-auto no-scrollbar pb-1">
              <TableFilter 
                label={statusFilter === "ALL" ? (locale === "ar" ? "كل الحالات" : "All states") : statusFilter} 
                onClick={() => {
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[950px] xl:min-w-full">
              <thead>
                <tr className="bg-slate-50/30">
                  <th className="w-[22%] px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest first:rounded-tl-3xl">
                    {locale === "ar" ? "اسم المريض" : "Patient Name"}
                  </th>
                  <th className="w-[18%] px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                    {locale === "ar" ? "الطبيب المعين" : "ASSIGNED DOCTOR"}
                  </th>
                  <th className="w-[15%] px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                    {locale === "ar" ? "التاريخ والوقت" : "DATE & TIME"}
                  </th>
                  <th className="w-[14%] px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                  ...
                  <th className="w-[10%] px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest text-center">
                    {locale === "ar" ? "التواصل" : "Comms"}
                  </th>
                  <th className="w-[11%] px-4 md:px-6 py-4 md:py-5 text-[10px] md:text-[11px] font-bold text-slate-400 uppercase tracking-widest text-right last:rounded-tr-3xl">
                  ...
                  Array.from({ length: 5 }).map((_, i) => (
                   ...
                   <tr>
                    <td colSpan={7} className="px-6 py-16 text-center text-slate-400 font-medium">
                      {locale === "ar" ? "لا توجد وصفات طبية" : "No prescriptions found"}
                    </td>
                  </tr>
                ) : (
                    ...
                    <tr key={item.id} className="hover:bg-slate-50/30 transition-colors group">
                      ...
                      <td className="px-4 md:px-6 py-4 md:py-6">
                        ...
                        <div className="text-[9px] md:text-[11px] font-bold text-slate-400 mt-0.5 uppercase tracking-tight truncate">
                          {new Date(item.createdAt).toLocaleTimeString(locale === "ar" ? "ar-EG" : "en-US", { hour: "2-digit", minute: "2-digit" })}
                        </div>
                      </td>
                      <td className="px-4 md:px-6 py-4 md:py-6">
          ...
          {filteredPrescriptions.length > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-4">
              <p className="text-[11px] md:text-[13px] font-bold text-slate-400">
                {locale === "ar" ? "عرض" : "Showing"} <span className="text-slate-900">1</span> {locale === "ar" ? "إلى" : "to"} <span className="text-slate-900">{Math.min(filteredPrescriptions.length, 10)}</span> {locale === "ar" ? "من" : "of"} <span className="text-slate-900">{filteredPrescriptions.length}</span> {locale === "ar" ? "نتيجة" : "results"}
              </p>
              <div className="flex items-center gap-2">
                ...
                <PaginationButton icon={ChevronRight} disabled={filteredPrescriptions.length <= 10} />
              </div>
            </div>
          )}
        </CardContent>
...
function TableFilter({ label, onClick }: { label: string; onClick?: () => void }) {
  return (
    <div 
      onClick={onClick}
      className="flex items-center gap-2 md:gap-4 px-3 md:px-5 py-2 md:py-3 bg-white border border-slate-100 rounded-xl md:rounded-2xl cursor-pointer hover:bg-slate-50 transition-colors whitespace-nowrap"
    >
