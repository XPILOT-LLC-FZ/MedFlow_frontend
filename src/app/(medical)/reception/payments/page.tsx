          ...
          <div className="bg-white rounded-[28px] shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6">
            <div className="flex flex-col sm:flex-row sm:items-center gap-6">
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-emerald-50 border border-emerald-100 rounded-2xl">
                <span className="text-[12px] font-bold text-slate-500">{isRTL ? "التأمين" : "INSURANCE"}</span>
                <span className="text-[12px] font-bold text-slate-900 ml-1">{(patient?.medicalHistory?.insuranceDetails as Record<string, unknown>)?.provider as string || (isRTL ? "لا يوجد تأمين" : "No Insurance")}</span>
                {(patient?.medicalHistory?.insuranceDetails as Record<string, unknown>)?.verificationStatus === "verified" ? (
                  <span className="text-[11px] font-black text-emerald-600 bg-emerald-100 px-2 py-0.5 rounded-lg ml-2">{isRTL ? "مفعل ✓" : "✓ Verified"}</span>
                ) : (
                  <span className="text-[11px] font-black text-amber-600 bg-amber-100 px-2 py-0.5 rounded-lg ml-2">{isRTL ? "قيد الانتظار" : "Pending"}</span>
                )}
              </div>
            </div>
            ...
            <div className="pt-4 border-t border-slate-100 space-y-3">
              ...
              <div className={cn("flex items-center justify-between", isRTL ? "flex-row-reverse" : "flex-row")}>
                <span className="text-[13px] font-bold text-emerald-600">
                  {isRTL ? "تغطية التأمين" : "Insurance Coverage"} ({discountPercent}%):
                </span>
                <span className="text-[14px] font-bold text-emerald-600">-{insuranceCoverage.toFixed(2)} {isRTL ? "ج.م" : "$"}</span>
              </div>
          ...
          <div className="bg-white rounded-[28px] shadow-[0_4px_20px_rgb(0,0,0,0.04)] p-6 space-y-5">
            ...
            <div className="space-y-3">
              ...
              <PaymentOption
                ...
                title={isRTL ? "تأمين" : "Insurance"}
                subtitle={isRTL ? "فوترة مباشرة" : "Direct Billing"}
              />
              <PaymentOption
