    ...
    <div dir={isRTL ? "rtl" : "ltr"} className="p-4 lg:p-8 space-y-10 bg-slate-50 min-h-screen pb-20 font-sans">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-10 gap-10">
        ...
        <div className="xl:col-span-7 space-y-8">
          ...
          <FormSection title={t("patientDetails")} icon={UserIcon}>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
               <InputField 
                 label={t("fullName").split(' ')[0]} 
                 placeholder={isRTL ? "الاسم الأول" : "First name"} 
                 value={formData.firstName} 
                 onChange={(e) => setFormData({ ...formData, firstName: e.target.value })} 
               />
               <InputField 
                 label={t("fullName").split(' ')[1] || "Last name"} 
          ...
          {/* 4. Emergency Contact */}
          <FormSection title={t("emergencyContacts")} icon={UserIcon}>
        ...
        <div className="xl:col-span-3 space-y-8">
          ...
          <Card className="border-none shadow-[0_4px_20px_rgb(0,0,0,0.01)] rounded-[24px] bg-white overflow-hidden p-6 space-y-6">
             <div className="space-y-1">
               <h3 className="text-[11px] font-black text-slate-300 uppercase tracking-widest">{t("similarRecords", { count: 2 })}</h3>
               <p className="text-[10px] font-bold text-slate-400 leading-tight">{t("checkingForExistingPatients")}</p>
             </div>
          ...
          <Card className="border-none shadow-[0_4px_20px_rgb(0,0,0,0.01)] rounded-[24px] bg-white overflow-hidden p-6 space-y-6">
             ...
             <div className="space-y-6 pl-1">
                <ProgressItem active completed label={t("personalDetails")} number={1} />
                <ProgressItem active label={t("insuranceAndCoverage")} number={2} />
                <ProgressItem label={t("emergencyContacts")} number={3} />
                <ProgressItem label={t("confirmation")} number={4} />
             </div>
          </Card>
        </div>
      </div>
...
function InputField({ label, placeholder, icon: Icon, value, onChange }: InputFieldProps) {
  ...
  return (
    <div className="space-y-2">
      <label className={cn("text-[12px] font-bold text-slate-400 uppercase tracking-widest", isRTL ? "mr-1" : "ml-1")}>{label}</label>
      <div className={cn("relative", isRTL && "text-right")}>
        <Input 
          placeholder={placeholder} 
          value={value}
          onChange={onChange}
          className={cn("h-12 rounded-xl border-slate-100 bg-slate-50/50 focus:ring-blue-600/5 transition-all text-sm font-medium", isRTL && "text-right")} 
        />
        ...
        <div className="xl:col-span-3 space-y-8">
          ...
          <Card className="border-none shadow-[0_4px_20px_rgb(0,0,0,0.02)] rounded-[28px] bg-white overflow-hidden">
            <CardContent className="p-8 space-y-8">
              ...
              <div className="space-y-6">
                <VitalRow emoji="❤️" label={t("heartRate")} value={vitals.heartRate || "--"} unit="BPM" color="text-red-500" />
                <VitalRow emoji="🩺" label={t("bloodPressure")} value={vitals.bp || "--"} unit="mmhg" color="text-blue-500" />
                <VitalRow emoji="🩸" label={isRTL ? "السكر" : "Glucose"} value={vitals.glucose || "--"} unit="mg/dL" color="text-orange-500" />
              </div>
            </CardContent>
          </Card>
